...
class SellerProducts extends StatefulWidget {
  SellerProducts(
      {Key? key, this.id, this.shop_name, this.catID, this.brandID, this.type})
      : super(key: key);
    ...
    if (widget.type == "filter") {
      if (widget.catID == 0) {
        ...
        productResponse = await ProductRepository()
            .getShopBrandWiseProductsFilter(
                id: widget.id ?? 0,
        ...
        productResponse = await ProductRepository().catBrandFilter(
            id: widget.id ?? 0,
            catID: widget.catID ?? 0,
            brandID: widget.brandID ?? 0,
            page: _page,
      ...
      print("inside else");
      productResponse = await ProductRepository()
          .getShopProducts(id: widget.id ?? 0, page: _page, name: _searchKey);
